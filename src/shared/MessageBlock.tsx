...
const Messages = ({ messageBlock, isSelf = false }: { messageBlock: MessageBlock, isSelf?: boolean }) => {
    const user = messageBlock.author;
    return <Container style={{ alignSelf: isSelf ? 'flex-end' : undefined }}>
        ...
        <Bubbles style={{ alignItems: isSelf ? 'flex-end' : 'flex-start'}}>
            ...
            {messageBlock.messages.map((msg: Message) => <Bubble key={msg.id}>{msg.text}</Bubble>)}
        </Bubbles>
    </Container>
};
